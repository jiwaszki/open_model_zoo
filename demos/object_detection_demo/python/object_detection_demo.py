...
def build_argparser():
    ...
    args.add_argument(
        "-at",
        "--architecture_type",
        help="Required. Specify model' architecture type.",
        type=str,
        required=True,
        choices=(
            "ssd",
    ...
    io_args.add_argument(
        "-o", "--output", required=False, help="Optional. Name of output to save."
    )
    io_args.add_argument(
        "-limit",
        "--output_limit",
    ...
    input_transform_args.add_argument(
        "--reverse_input_channels",
        default=False,
        action="store_true",
        help="Optional. Switch the input channels order from " "BGR to RGB.",
    )
    input_transform_args.add_argument(
        "--mean_values",
        default=None,
...
def get_model(ie, args):
    input_transform = models.InputTransform(
        args.reverse_input_channels, args.mean_values, args.scale_values
    )
    common_args = (ie, args.model, input_transform)
    if (
        args.architecture_type in ("ctpn", "yolo", "yolov4", "retinaface")
        and not input_transform.is_trivial
    ):
        raise ValueError(
            "{} model doesn't support input transforms.".format(args.architecture_type)
        )
    ...
    if args.architecture_type == "ssd":
        ...
        return models.CTPN(
            ie, args.model, input_size=args.input_size, threshold=args.prob_threshold
        )
        ...
        return models.CenterNet(
            *common_args, labels=args.labels, threshold=args.prob_threshold
        )
        ...
        return models.UltraLightweightFaceDetection(
            *common_args, threshold=args.prob_threshold
        )
    else:
        raise RuntimeError(
            "No model type or invalid model type (-at) provided: {}".format(
        ...
        if detection.score > threshold:
            ...
            if isinstance(detection, models.DetectionWithLandmarks):
                for landmark in detection.landmarks:
                    ...
                    cv2.circle(
                        frame, (int(landmark[0]), int(landmark[1])), 2, (0, 255, 255), 2
                    )
    return frame
        ...
        if detection.score > threshold:
            ...
            log.info(
                "{:^9} | {:10f} | {:4} | {:4} | {:4} | {:4} ".format(
                    det_label, detection.score, xmin, ymin, xmax, ymax
                )
            )
...
def main():
    ...
    log.info("Loading network to {} plugin...".format(args.device))

    exec_net = ie.load_network(
        network=model.net, device_name=args.device, config=plugin_config
    )

    infer_queue = AsyncPipeline(
        network=exec_net, model=model, jobs=args.num_infer_requests
    )
    ...
    while True:
        if infer_queue.callback_exceptions:
            raise infer_queue.callback_exceptions[0]
        # Process all completed requests
        results = infer_queue.get_result(next_frame_id_to_show)
        ...
        if results:
            ...
            if len(objects) and args.raw_output_message:
                print_raw_results(
                    frame.shape[:2], objects, model.labels, args.prob_threshold
                )
            ...
            if video_writer.isOpened() and (
                args.output_limit <= 0 or next_frame_id_to_show <= args.output_limit - 1
            ):
        ...
        if infer_queue.is_ready():
            ...
            if frame is None:
                ...
                break
            if next_frame_id == 0:
                output_transform = models.OutputTransform(
            ...
            infer_queue.async_infer(
                inputs=frame,
                userdata=[next_frame_id, {"frame": frame, "start_time": start_time}],
            )
            next_frame_id += 1
    ...
    for next_frame_id_to_show in range(next_frame_id_to_show, next_frame_id):
        ...
        presenter.drawGraphs(frame)
        frame = draw_detections(
            frame, objects, palette, model.labels, args.prob_threshold, output_transform
        )
        metrics.update(start_time, frame)
